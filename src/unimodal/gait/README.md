...
```bash
cd /ocean/projects/med260006p/shared/biomedAI/project/src/unimodal/gait
python train_gait.py
```

## Running On Bridges2 (SLURM)

Use the new SLURM scripts in `src/unimodal/gait/slurm/`.

GPU path (recommended for TCN + WearGait + RF + fusion):
...
To override GPU type/count at submit time (must obey partition limits):

```bash
sbatch --gpus=v100-32:1 src/unimodal/gait/slurm/gait_pipeline_gpu_shared.slurm
```

Bridges2 limits relevant to these scripts:
- `GPU-shared`: up to 4 GPUs/job, up to 48h runtime
- `RM-shared`: one node, up to 64 cores, up to 72h runtime

The GPU script includes a runtime guard that exits if more than 4 GPUs are requested in `GPU-shared`.
