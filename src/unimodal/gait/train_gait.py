...
TASK_OUTPUT_DIR = OUTPUT_DIR / "PDFE_Severity_Classification"
...
def main():
    ...
    # Save scaler
    with open(TASK_OUTPUT_DIR / 'scaler.pkl', 'wb') as f:
        pickle.dump(scaler, f)
    print(f"Saved scaler to {TASK_OUTPUT_DIR / 'scaler.pkl'}")
    ...
    for fold, (train_idx, val_idx) in enumerate(sgkf.split(X_scaled, all_labels, groups=all_subjects)):
        ...
        fold_dir = TASK_OUTPUT_DIR / f'fold_{fold+1}'
        ...
        oof_window_subjects.extend(val_subjects.tolist())
        oof_window_y_true.extend(np.array(val_labels).tolist())
        oof_window_y_pred.extend(np.array(val_preds).tolist())
        oof_window_proba_1.extend(np.array(val_probs).tolist())
    ...
    # Save results
    results_df.to_csv(TASK_OUTPUT_DIR / 'cv_results.csv', index=False)

    # Aggregate out-of-fold window predictions into subject-level predictions.
    subj_prob_store = {}
    subj_label_store = {}
    for sid, y_t, p1 in zip(oof_window_subjects, oof_window_y_true, oof_window_proba_1):
        sid = str(sid)
        subj_prob_store.setdefault(sid, []).append(float(p1))
        subj_label_store[sid] = int(y_t)

    subject_ids = sorted(subj_prob_store.keys())
    y_true_subject = np.array([subj_label_store[sid] for sid in subject_ids], dtype=np.int64)
    y_prob_subject_1 = np.array([np.mean(subj_prob_store[sid]) for sid in subject_ids], dtype=np.float32)
    y_pred_subject = (y_prob_subject_1 >= 0.5).astype(np.int64)
    y_proba_subject = np.column_stack([1.0 - y_prob_subject_1, y_prob_subject_1]).astype(np.float32)

    np.savez_compressed(
        TASK_OUTPUT_DIR / 'predictions.npz',
        subject_ids=np.array(subject_ids, dtype='U32'),
        y_true=y_true_subject,
        y_pred=y_pred_subject,
        y_proba=y_proba_subject,
        window_subject_ids=np.array(oof_window_subjects, dtype='U32'),
        window_y_true=np.array(oof_window_y_true, dtype=np.int64),
        window_y_pred=np.array(oof_window_y_pred, dtype=np.int64),
        window_y_proba=np.column_stack([
            1.0 - np.array(oof_window_proba_1, dtype=np.float32),
            np.array(oof_window_proba_1, dtype=np.float32),
        ]).astype(np.float32),
    )
    ...
    summary = {
        ...
        'prediction_level': 'subject_from_oof_windows',
        'outputs': {
            'cv_results_csv': str(TASK_OUTPUT_DIR / 'cv_results.csv'),
            'predictions_npz': str(TASK_OUTPUT_DIR / 'predictions.npz'),
            'scaler_pkl': str(TASK_OUTPUT_DIR / 'scaler.pkl'),
        },
    }
