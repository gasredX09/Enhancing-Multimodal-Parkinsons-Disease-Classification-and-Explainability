...
def infer_predictions(model, x: np.ndarray, batch_size: int, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    ds = WindowDataset(x=x, y=np.zeros((len(x),), dtype=np.int64))
    dl = DataLoader(ds, batch_size=batch_size, shuffle=False)

    prob_1_list: List[np.ndarray] = []
    ...
    with torch.no_grad():
        for xb, _ in dl:
            ...
            prob_1_list.append(probs.cpu().numpy())
            pred_list.append(preds.cpu().numpy())

    if not prob_1_list:
        return np.zeros((0,), dtype=np.float32), np.zeros((0,), dtype=np.int64)

    return np.concatenate(prob_1_list).astype(np.float32), np.concatenate(pred_list).astype(np.int64)
...
def main() -> None:
    ...
    subject_prob_store: Dict[str, List[float]] = {}
    subject_pred_store: Dict[str, List[int]] = {}
    subject_label_store: Dict[str, int] = {}
    ...
    for fold, (tr, va) in enumerate(sgkf.split(X, y, groups), start=1):
        ...
        prob_va_1, pred_va = infer_predictions(model, X_va, batch_size=args.batch_size, device=device)
        for idx_local, emb in enumerate(emb_va):
            ...
            subject_prob_store.setdefault(subject_id, []).append(float(prob_va_1[idx_local]))
            subject_pred_store.setdefault(subject_id, []).append(int(pred_va[idx_local]))
            subject_label_store[subject_id] = int(y_va[idx_local])
    ...
    subject_ids = sorted(subject_prob_store.keys())
    y_true_subject = np.array([subject_label_store[sid] for sid in subject_ids], dtype=np.int64)
    y_prob_subject_1 = np.array([np.mean(subject_prob_store[sid]) for sid in subject_ids], dtype=np.float32)
    ...
    pred_npz = args.output_dir / "predictions.npz"
    np.savez_compressed(
        pred_npz,
        subject_ids=np.array(subject_ids, dtype="U64"),
